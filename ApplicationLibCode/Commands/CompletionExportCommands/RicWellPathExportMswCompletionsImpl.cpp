        ...
        for ( std::shared_ptr<RicMswCompletion> completion : segment->completions() )
        {
            if ( !completion->subSegments().empty() && exportCompletionTypes.count( completion->completionType() ) )
            {
                ...
                for ( const std::shared_ptr<RicMswSegment>& subSegment : completion->subSegments() )
                {
                    ...
                    for ( const std::shared_ptr<RicMswSegmentCellIntersection>& intersection :
                ...
                for ( size_t nSubValve = 0u; nSubValve < valve->valveLocations().size(); ++nSubValve )
                {
                    ...
                    if ( segment->startMD() <= valveMD && valveMD < segment->endMD() )
                    {
                        if ( valve->componentType() == RiaDefines::WellPathComponentType::AICD )
                        {
                            ...
                            auto subSegment =
                                std::make_shared<RicMswSegment>( "Valve segment", exportStartMD, exportEndMD, exportStartTVD, exportEndTVD );
                                                                                  ...
                                                                                  bool* foundSubGridIntersections )
{
    ...
    std::shared_ptr<RicMswSegment> subSegment( new RicMswSegment( "Fracture segment", position, position + width, 0.0, 0.0 ) );
